...
from graphGB import Graph
from context import books, reviews, interactions
...
class RecSystem1(Graph):
    ...
    def similar_users(self,userId: int, threshold: float)->set:
        ...
        '''
        user = self.get_userNode_nodeId(userId)
        simUsersId = set()
...
class ResultsRS1:
    ...
    def show_recommended_book(self, userId: int, threshold:float):
        '''
        print recommended movie in the console
        '''
        movie, nbRec = self.rs1.recommend_book(userId = userId, threshold = threshold)
...
if __name__ == '__main__':
    res = ResultsRS1()
    res.show_recommended_book(50,0.9)
